 ...
 *         01.08.2017.
 */
public class AddLocalizedGenreInfo implements Command {
    private final static Logger logger = Logger.getLogger(AddLocalizedGenreInfo.class);
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            boolean isAdded = genreService.addLocalizedGenreInfo(genreId,
                    name, description, contentLanguage);
        } catch (ServiceException e) {
            logger.error("Error during AddLocalizedGenreInfo", e);
