 ...
 *         27.07.2017.
 */
public class GetGenresInMovieCommand implements Command {
    private final static Logger logger = Logger.getLogger(GetGenresInMovieCommand.class);
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            List<Genre> genreList =
                    genreService.getGenresByMovieId(movieId, currentLanguage);
            String genres = new Gson().toJson(genreList);
            response.setContentType(CONTENT_TYPE);
            response.setCharacterEncoding(ENCODING);
            response.getWriter().write(genres);
