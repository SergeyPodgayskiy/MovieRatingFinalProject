 ...
 *         01.08.2017.
 */
public class GetLocalizedGenreInfoCommand implements Command {
    private static final Logger logger = Logger.getLogger(GetLocalizedGenreInfoCommand.class);
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            Genre genre = genreService.getLocalizedGenreInfoByCode(genreId, languageCode);
            String MovieJson = new Gson().toJson(genre);
            response.setContentType(CONTENT_TYPE);
            response.setCharacterEncoding(ENCODING);
            response.getWriter().print(MovieJson);
        } catch (ServiceException e) {
            logger.error("Error during GetLocalizedGenreInfoCommand", e);
