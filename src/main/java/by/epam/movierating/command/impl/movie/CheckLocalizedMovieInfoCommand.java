 ...
 *         26.07.2017.
 */
public class CheckLocalizedMovieInfoCommand implements Command {
    private static final Logger logger = Logger.getLogger(CheckLocalizedMovieInfoCommand.class);
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            boolean isExist = movieService.checkLocalizedMovieInfo(movieId, languageCode);
            String isExistJson = new Gson().toJson(isExist);
            response.setContentType(CONTENT_TYPE);
            response.setCharacterEncoding(ENCODING);
            response.getWriter().write(isExistJson);
        } catch (ServiceException e) {
            logger.error("Error during CheckLocalizedMovieInfoCommand", e);
