 ...
 *         26.07.2017.
 */
public class GetLanguageMovieInfoByCodeCommand implements Command {
    private static final Logger logger = Logger.getLogger(GetLanguageMovieInfoByCodeCommand.class);
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            Movie movie = movieService.getLanguageMovieInfoByCode(movieId, languageCode);
            String MovieJson = new Gson().toJson(movie);
            response.setContentType(CONTENT_TYPE);
            response.setCharacterEncoding(ENCODING);
            response.getWriter().print(MovieJson);
        } catch (ServiceException e) {
            logger.error("Error during GetLanguageMovieInfoByCodeCommand", e);
