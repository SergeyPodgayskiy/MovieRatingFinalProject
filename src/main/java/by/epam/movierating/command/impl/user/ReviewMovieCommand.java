...
public class ReviewMovieCommand implements Command {
    private final static Logger logger = Logger.getLogger(ReviewMovieCommand.class);
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            boolean isAdded =
                    reviewService.reviewMovie(idMovie,idUser,title,text,type);
            String json = new Gson().toJson(isAdded);
            response.setContentType(CONTENT_TYPE);
            response.setCharacterEncoding(ENCODING);
            response.getWriter().write(json);
        } catch (ServiceException e) {
            logger.error("Error during executing ReviewMovieCommand", e);
